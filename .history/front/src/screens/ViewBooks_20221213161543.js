...
export default Viewbooks = ({ navigation }) => {
  ...
  function deleteBook(book) {
    setbook(book);
    const response = axios.delete(`/books/${user.id}`);
    listbooks();
}
    ...
    <SafeAreaView style={theme.safeArea}>
      <View style={theme.container}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <ItemBook item={item} alterBook={() => alterBook(item)} deleteBook={() => deleteBook(item)} />
        )}
        />

        <FloatingButton icon="plus" color="#333" onPress={() => newbook()} />

        <Modalize ref={modalRef} snapPoint={620} modalHeight={height * 0.8}>
          <KeyboardAvoidingView
